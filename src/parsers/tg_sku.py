from dataclasses import dataclass
from datetime import datetime
from loguru import logger
from telethon.tl.types import MessageEntityTextUrl, PeerChat, PeerChannel
from src.dao.mentions_db import ParserResultTgPost, Sku, SkuPerPost, ChatContentType
from src.utils.wb_utils import wb_sku_pattern, wb_size_pattern
from src.parsers.tg_abstract import AbstractTgChatParser, AbstractParserTgChatResult
from src.dao.mentions_db import TgChatsToParse
from src.utils import resolve_redirection_link


class TgWbItemsAdChatParser(AbstractTgChatParser):

    chats_type = ChatContentType.wb_items_ads

    def __init__(self, session_id: int, tg_chats_to_parse: list[TgChatsToParse], start_date: datetime):
        super().__init__(session_id, tg_chats_to_parse, start_date)
        self.parsed_tg_chats = set()
        self.parsed_sku_db_instances = dict()

    def parse_message(self, message):
        parsed_tg_chats_from_message = super().parse_message(message)
        self.parsed_tg_chats = self.parsed_tg_chats.union(parsed_tg_chats_from_message)

        if message.fwd_from is not None:  # skip if message is reply
            return

        skus = self.get_skus_from_message_hyperlinks(message)
        skus_a = {int(s) for s in wb_sku_pattern.findall(message.message) if s is not None}
        skus = skus.union(skus_a)
        skus = skus.difference([int(s) for s in wb_size_pattern.findall(message.message)])

        chat_id = None
        if isinstance(message.peer_id, PeerChat):
            chat_id = message.peer_id.chat_id
        elif isinstance(message.peer_id, PeerChannel):
            chat_id = message.peer_id.channel_id
        if chat_id is None:
            return set()

        if len(skus) == 0:
            return set()

        date = message.date                                                     # дата публикации
        forwards_count = message.forwards or 0  # int                           # количество пересылок (поделились)
        views_count = message.views or 0        # int or None if it is chat     # просмотры
        reactions_count = 0                                                     # реакции если есть
        if message.reactions is not None:
            for reaction_count_entity in message.reactions.results:
                reactions_count = reactions_count + reaction_count_entity.count

        if message.replies is not None:
            replies_count = message.replies.replies or 0                        # комментарии
        else:
            replies_count = 0

        followers = self.get_followers_by_chat_id(chat_id)                      # подпосчики
        er = (replies_count + reactions_count) / followers * 100                # er = (комменты+реакции)/подпищеки*100

        post = ParserResultTgPost(message_id=str(message.id), chat_id=str(chat_id), views_count=views_count,
                                  replies_count=replies_count, shared_count=forwards_count, er=er,
                                  reactions_count=reactions_count, comments_count=replies_count, date=date)

        for sku in skus:
            if sku not in self.parsed_sku_db_instances.keys():
                sku_db_instance = Sku(sku_code=sku)
                self.parsed_sku_db_instances[sku] = sku_db_instance
            else:
                sku_db_instance = self.parsed_sku_db_instances[sku]

            sku_db_instance.sku_per_post.append(SkuPerPost(sku_code=sku, post=post))
        return {post}

    def get_followers_by_chat_id(self, tg_chat_id: int) -> int:
        for tg_chat in self.tg_chats_to_parse:
            if tg_chat.tg_id == str(tg_chat_id):
                return tg_chat.followers

    def get_skus_from_message_hyperlinks(self, message) -> set[str]:
        skus = set()
        for url_entity, inner_text in message.get_entities_text(MessageEntityTextUrl):
            # <editor-fold desc="log">
            logger.debug(f'INNER TEXT: {inner_text}; LINK: {url_entity}')
            # </editor-fold>
            resolved_sku = resolve_redirection_link(url_entity.url)
            if resolved_sku is not None:
                skus = skus.union([resolved_sku])
        return skus

    def get_parsed_results(self):
        return TgWbItemsAdChatParserResult(self.get_tg_chats_to_update(), self.parsed_items,
                                           self.parsed_sku_db_instances)


@dataclass
class TgWbItemsAdChatParserResult(AbstractParserTgChatResult):
    """
    Wrapper for parser results
    """
    parsed_posts: set
    parsed_skus: dict

    def merge_with(self, another_parser_result):
        super().merge_with(another_parser_result)
        self.parsed_posts = self.parsed_posts.union(another_parser_result.parsed_posts)
        self.parsed_skus.update(another_parser_result.parsed_skus)

    def get_parsed_items_count(self):
        """
        :return: amount of parsed mentions
        """
        return len(self.parsed_skus)
